...
class Class extends Entity {
    ...
    async loadInfo() {
        if (!this._infoLoaded) {
            ...
            this._name = infos.class.name;
            this._xmiID = infos.class.xmiID;
            this._visibility = Entity.convertVisibility(infos.class.visibility);
            this._diagramID = infos.class.diagramID;
     ...
     * @throws InfosAlreadyLoadedException if the class infos are already loaded.
     */
    // async loadInfo(alreadyLoadedInfos = null) {
    //     let infos;
    //     if (!this._infoLoaded) {
    //         if (!alreadyLoadedInfos) {
    //             infos = await ClassWrapper.getInfo(this.id).class;
    //         } else infos = alreadyLoadedInfos;
    //         this._name = infos.name;
    //         this._xmiID = infos.xmiID;
    //         this._visibility = Entity.convertVisibility(infos.visibility);
    //         this._diagramID = infos.diagramID;
    //         let attributesInfo = info.attributes;
    //         let methodsInfo = info.methods;
    //         for (let i = 0; i < attributesInfo.length; i++) {
    //             let attributeInfo = attributesInfo[i];
    //             let attrID = attributeInfo.id;
    //             this._attributes.idList.push(attrID);
    //             this._attributes[attrID] = new Attribute(attrID, false);
    //             this._attributes[attrID].loadInfo(attributeInfo);
    //         }
    //
    //         for (let i = 0; i < methodsInfo.length; i++) {
    //             let methodInfo = methodsInfo[i];
    //             let methodID = methodInfo.id;
    //             this._methods.idList.push(methodID);
    //             this._methods[methodID] = new Method(methodID, false);
    //             this._methods[methodID].loadInfo(methodID);
    //         }
    //
    //         this._infoLoaded = true;
    //     } else throw new InfosAlreadyLoadedException('class');
