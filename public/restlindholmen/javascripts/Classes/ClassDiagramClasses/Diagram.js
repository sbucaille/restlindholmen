...
class Diagram extends Entity {
    ...
    async loadInfo() {
        if (!this._infoLoaded) {
            ...
            this._fileName = info.diagram.filePath;
    ...
    //             info = await DiagramWrapper.getInfo(this.id).diagram;
    //         } else info = alreadyLoadedInfos.diagram;
    //         this._fileName = info.filePath;
    //         let classesInfo = info.classes;
    //         for (let i = 0; i < classesInfo.length; i++) {
    //             let classInfo = classesInfo[i];
    //             this._classes.idList.push(classInfo.id);
    //             this._classes[classInfo.id] = new Class(classInfo.id, false);
    //             this._classes[classInfo.id].loadInfo(classInfo);
    //         }
    //         this._infoLoaded = true;
    //     } else {
    //         throw new InfosAlreadyLoadedException('diagram');
